...
/*   Updated: 2023/07/05 21:46:06 by jgermany         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	colorize_pixel(int start, t_img *img_con, uint32_t color)
{
	int	i;
...
int	test_mandelbrot(int x, int y)
{
	double complex z;
	int	i;
	...
	i = -1;
	z = 0;
	while (++i < 4) // creal(z) <= 10 && cimg(z) <= 10
	{
...
int	draw_on_scene(t_mlx *mlx_data, t_img *img_con)
{
	...
	// y = -1;
	colorize_pixel(0 * (img_con->bpp / 8) + 0 * img_con->szl,
		img_con, WHITE);
	// while (++y < WINDOW_Y)
	// {
	// 	x = -1;
